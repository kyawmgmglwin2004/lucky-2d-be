...
async function updateAllNumberDetail(req, res) {
    try {
        const { rate, round_rate, status_limit_amounts, real_limit_amounts } = req.body;
        const serviceRes = await twoDService.updateAllNumberDetails(rate, round_rate, status_limit_amounts, real_limit_amounts);
...
async function updateNumberDetailById(req, res) {
    try {
        ...
        const { rate, round_rate, status_limit_amounts, real_limit_amounts, status } = req.body;
        const serviceRes = await twoDService.updateNumberDetailById(id, rate, round_rate, status_limit_amounts, real_limit_amounts, status);
        return res.status(serviceRes.code).json(serviceRes);
    } catch (error) {
